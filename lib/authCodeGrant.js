...
function generateCode (done) {
  var self = this;
  var codeType = self.responseType === 'code' ?
    'authorization_code' : self.responseType;
...
function saveToken(done) {
  ...
  this.model.saveAccessToken(this.token, this.client.clientId, expires,
...
function saveAuthCode (done) {
  var nonce = this.req.session.nonce || '',
    expires = new Date();
...
function save(done) {
  if(this.responseType==='token') {
    ...
    saveAuthCode.call(this,done);
  }
}


function setRedirectParams(done) {
  if(this.responseType === 'code') {
    this.redirectParams = {key: 'code',separator: '?',value: this.authCode};
  } else {
    this.redirectParams = {key: 'access_token',separator: '#',value: this.token};
  }
  done();
}
