 ...
 * @this   OAuth
 */
function generateIdToken (done) {
  var d = Math.round(new Date().getTime()/1000);
  this.id_token = {
    iss: this.req.protocol + '://' + this.req.headers.host,
    sub: this.user.id,
    aud: this.req.body.client_id,
    exp: d + 3600,
    iat: d,
    nonce: this.nonce || ''
  };
...
function sendResponse (done) {
  ...
  if (this.accessToken && this.id_token) {
    response.id_token = this.id_token;
    var hbuf = crypto.createHmac('sha256', this.client.clientSecret).update(this.accessToken).digest();
    response.id_token.ht_hash = base64url(hbuf.toString('ascii', 0, hbuf.length/2));
    response.id_token = jwt.encode(response.id_token, this.client.clientSecret);
  }
